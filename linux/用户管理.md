...
# useradd 修改其配置文件（/etc/passwd）来添加用户，如果用户不指定某些选项，其默认值主要参考配置文件 /etc/default/useradd  和 /etc/login.defs
```
...
很高兴听到您对 `setuid` 和 `setgid` 权限有了清晰的理解！现在让我们来探讨 `chattr` 和 `lsattr` 命令，这两个命令在 Linux 系统中用于管理文件的特殊属性。
