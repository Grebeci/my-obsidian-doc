...
- ACK，
  ...
  - `acks=1`（默认值）：生产者会等待leader副本确认消息已写入其日志，但不等待所有follower副本的确认。如果消息发送到leader后未收到确认（例如，因为leader突然宕机），生产者将重试发送（取决于  retries 配置）。

  - `acks=all`（或`acks=-1`）：生产者会等待所有同步副本（leader和所有活跃的follower）确认。这提高了消息不丢失的概率，但如果不满足最小ISR（In-Sync Replicas）要求，会增加重试的情况（取决于  retries 配置）。
