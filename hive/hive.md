...
Hive 不需要集群化部署，只需要考虑一下组件的部署

1. **Hive CLI (Command Line Interface)**: 主要提供 `bin/hive` 和 `Beeline` 两种 Hive 客户端。以前台会话的方式运行 HQL 查询。
   
2.  **HiveServer2**: 这是 Hive 提供的服务端组件，允许远程客户端（如 JDBC/ODBC 客户端、BI 工具等）连接并执行 Hive 查询。它是一个独立的进程。
   
3. **Metastore Service**: Hive Metastore 是 Hive 的元数据存储服务。它可以运行在两种模式下：
